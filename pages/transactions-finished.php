  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        ...
        <li class="nav-item has-treeview">
            <a href="transactions-finished.php" class="nav-link active">
              <i class="nav-icon fa fa-money"></i>
              <p>
        ...
        <table>
          <tr>
            ...
            <th>Current Charge</th>
					...
					<?php
					$result = $mysqli->query("SELECT rental.rental_startdate,rental.rental_enddate,(reservation.reservation_fee + rental.rental_fee) as 'current charge', house.house_name, (SELECT CONCAT(given_name, ' ',last_name) FROM users WHERE user_id = house.user_id) as 'owner', (SELECT CONCAT(given_name, ' ',last_name) FROM users WHERE user_id = reservation.user_id) as 'customer' FROM reservation JOIN rental ON reservation.reservation_id = rental.reservation_id JOIN house ON reservation.house_id = house.house_id JOIN users ON reservation.user_id = users.user_id") or die ($mysqli->error());
